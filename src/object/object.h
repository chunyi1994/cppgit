...
inline Object::Object(enum Type type, std::string content):
    type_(type),
    content_(content),
    hash_()
{
...
inline const std::string &Object::Hash() const {
    return hash_;
}

inline Error Object::Sha1Hash(const std::string& src, std::string* dest) {
    assert(dest != nullptr);
    CSHA1 sha1;
...
inline std::string Object::TypeString(enum Object::Type t) {
    switch (t) {
    case BLOB:
        return "blob";
    case COMMIT:
